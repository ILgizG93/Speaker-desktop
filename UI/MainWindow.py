...
from PySide6.QtWidgets import QMainWindow, QWidget, QVBoxLayout, QTableView, QHBoxLayout, QHeaderView, QPushButton, QItemDelegate, QAbstractItemView
from PySide6.QtCore import QObject, Qt, QTimer, QSize
...
class SpeakerApplication(QMainWindow):
    def __init__(self, settings, interface):
        ...
        self.btn_sound_create.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        self.btn_sound_play.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        self.btn_sound_stop.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        self.btn_sound_delete.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        self.main_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)
        self.main_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)
        self.main_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.ResizeMode.ResizeToContents)
        self.main_table.horizontalHeader().setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)
        self.main_table.horizontalHeader().setSectionResizeMode(5, QHeaderView.ResizeMode.Stretch)
        self.main_table.horizontalHeader().setSectionResizeMode(6, QHeaderView.ResizeMode.ResizeToContents)
        self.main_table.horizontalHeader().setSectionResizeMode(7, QHeaderView.ResizeMode.Stretch)
        self.main_table.horizontalHeader().setSectionResizeMode(8, QHeaderView.ResizeMode.Fixed)
        self.main_table.horizontalHeader().setSectionResizeMode(9, QHeaderView.ResizeMode.Fixed)
        self.main_table.horizontalHeader().setSectionResizeMode(10, QHeaderView.ResizeMode.Fixed)
        ...
        self.main_table.selectionModel().selectionChanged.connect(self.get_current_row_id)
        ...
        self.table_layout_buttons.setAlignment(Qt.AlignmentFlag.AlignLeft)
    ...
    def refresh_schedule_table(self, data, network_reply=QtNetwork.QNetworkReply.NetworkError.NoError):
        ...
        if (len(self.schedule_data) == 0):
            self.schedule_data = [(None,) * len(self.schedule_header)]
        self.main_table_model.setItems(self.schedule_data)
    ...
    def set_active_row(self) -> None:
        if self.current_row_id:
            ...
            self.current_row_id = self.main_table.model().index(0, 0).data()
    ...
    def playlist_prepare(self, schedule_id):
        ...
        playlist.append(['_'.join((schedule_id, key)) for key, language in languages_sorted.items() if language.get('display')])
        print(playlist)
        return playlist
    ...
    def play_sound(self, is_next: bool=False) -> None:
        ...
        schedule_id = self.current_row_id
        if schedule_id:
            if not self.playlist and not is_next:
                self.playlist = self.playlist_prepare(schedule_id)
            ...
            if not os.path.isfile(file_name):
                ...
                raise SpeakerException("Ошибка воспроизведения", "Файл не найден.")
